...
This does not need to include trends that only last 1 hour.

# The trends need to be distincted and looked up so ALL tweets with thhat trend are used for top tweet, not just a few.
...
```{r}
desired_date_range <- seq(Sys.Date(), as.Date("2020-06-01"), by = -1)
```
...
```{r}
#write_csv(tweets_df, "june_tweets.csv")
#write_csv(trends_df, "june_trends.csv")
```
...
# On click, go to a popular tweet
# TF-IDF on hover
